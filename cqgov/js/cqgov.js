...
}
{
    ...
    function rr(){
        $("#linkUl>li").on("mouseenter",function(){
            let _this = $(this);
            let index = _this.index();
            let x = $("#xx");
            let bk = x.children(".bk");
            x.stop().animate({
                height:"200px"
            },200);
            _this.addClass("linkon");
            _this.siblings().removeClass("linkon");
            bk.eq( index ).show();
            bk.eq( index ).siblings().hide();
        });
        $("#interLink").on("mouseleave",function(){
            let _this = $("#linkUl>li");
            let index = _this.index();
            let x = $("#xx");
            let bk = x.children(".bk");
            _this.addClass("linkoff");
            x.stop().animate({
                height:"0"
            },200);
            bk.hide();
